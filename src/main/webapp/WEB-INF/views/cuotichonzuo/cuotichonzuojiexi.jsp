...
<body>
    ...
    //页面加载时就会加载这个函数，然后会调用下面的方法
      ...
      $(function () {//
        ...
        //选项的个数
        var  count=0;
         function getStrCountarmstr(abcdeq,armstr){//scrstr 源字符你 串 armstr 特殊字符
       	  /* alert("进入函数"); */
      ...
      function  Jexi(knowledge_Name,datas,datalength){
		        	...
		        	window.location.href="../cuotijiexi/detailspage?correctnumber="+correctnumber+"&percentage="+percentage+"&int_second="+int_second+"&knowledge_Name="+subListname+"&queanswer="+queanswer+"&answers="+answers+"&nickname="+nickname+"&openid="+openid+"&headimgurl="+headimgurl+"&tixing="+tixing+"&jiexihuidiao="+jiexihuidiao+"&days="+days+"&dateTime="+dateTime+"&integral="+integral+"&lv1state="+lv1state+"&lv2state="+lv2state+"&lv3state="+lv3state+"&knowledge_Id="+knowledge_Id+"&datalength="+datalength+"&kemuZYHXY="+kemuZYHXY+""; 
        		}
        ...
        function quedivshow() {//
    		...
    		var storagexuanxian = window.localStorage;//window.localStorage，H5的页面缓存
  			...
  			console.log("正确选择："+storagexuanxian["cuotianswers"+knowledge_Id]);
	 		console.log("用户选择的答案："+storagexuanxian["cuotiqueanswer"+knowledge_Id]);
