    ...
    //页面加载时就会加载这个函数，然后会调用下面的方法
       ...
       //页面加载时就会加载这个函数，然后会调用下面的方法
          $(function () {
         	...
         	ajaxjson(knowledge_Id,kemuZYHXY,openid); 
             /* show_time(); */
         	/* showTimeIncrease(); */
         });   
     ...
     //选项的个数
        ...
        function  ajaxjson(knowledge_Id,kemuZYHXY,openid){
			$.ajax({
      ...
      function  Jexi(knowledge_Name,datas,datalength){
		        	...
		        	window.location.href="../lishicuotijiexi/detailspage?correctnumber="+correctnumber+"&percentage="+percentage+"&int_second="+int_second+"&knowledge_Name="+subListname+"&queanswer="+queanswer+"&answers="+answers+"&nickname="+nickname+"&openid="+openid+"&headimgurl="+headimgurl+"&tixing="+tixing+"&jiexihuidiao="+jiexihuidiao+"&days="+days+"&dateTime="+dateTime+"&integral="+integral+"&lv1state="+lv1state+"&lv2state="+lv2state+"&lv3state="+lv3state+"&knowledge_Id="+knowledge_Id+"&datalength="+datalength+"&kemuZYHXY="+kemuZYHXY+""; 
        		}
