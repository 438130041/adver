...
<body>
            ...
            <div class="btndiv">
                <div class="row">
                    <div class="col-xs-4">
                     ...
                     <div class="col-xs-4">
                    	<div page="" id="fanghui" class="btn btn-warning btnanniu" onclick="fanghui()">返回</div>
	...
	var  lv3state = $(".lv3state").text();
	 ...
	 function   fanghui(){//返回
		 ...
		 /*  alert("科目："+kemu);  */
		 window.location.href="../timushocangzhangjie/timushocangzhangjie?kemu="+kemu+"&kemuZYHXY="+kemuZYHXY+"&openid="+openid+"&headimgurl="+headimgurl+"&nickname="+nickname+"&days="+days+"&integral="+integral+"&dateTime="+dateTime+"&lv1state="+lv1state+"&lv2state="+lv2state+"&lv3state="+lv3state+"";
	 }
	 ...
	 //页面加载时就会加载这个函数，然后会调用下面的方法
       ...
       //选项的个数
        ...
        function  ajaxjson(knowledge_Id,kemuZYHXY){
			$.ajax({
					url:'../timushocangzhangjie/practice',
					dataType:'json',
					type:'post',
					data:{knowledge_Id:knowledge_Id,kemuZYHXY:kemuZYHXY,openid:openid}, 
				...
				if(locationABCDE["timushocang"+"que"+knowledge_Id]  != null ){
				...
				//layer.msg('的确很重要', {icon: 1});
					$('.quediv').hide();
	 				console.log(locationABCDE["timushocang"+"que"+knowledge_Id]);
	 				var quest = locationABCDE["timushocang"+"que"+knowledge_Id].replace(knowledge_Id,"");//最后做到的题
	  			    $("#"+quest).show();
	 				var  size = quest.replace("que","");
					var  width = 1/datalength*100;//进度条初始值为1除以总数乘以100
    				var  widths = width*size;//每页进度占比等于初始值乘以页码
    				pagesq = size;//做到了第多少题
    				$("#my").text(locationABCDE["timushocang"+"que"+pagesq+"answer"+knowledge_Id]);
    				$("#correct").text( locationABCDE["timushocang"+"answers"+pagesq+knowledge_Id]);
    				/* Dpagesq = size; */
    				...
    				queids = pagesq;//当前第几到题
 					...
 					for(var i = 1;i<= datalength;i++){
 						for(var  k = 0;k<5;k++){
 							if(locationcolor["timushocang"+"que"+i+"1"+ABCD[k]+knowledge_Id] != null){//添加缓存颜色
 	 							$("[id = "+"timushocangque"+i+"1"+ABCD[k]+"]").prop("class",locationcolor["timushocang"+"que"+i+"1"+ABCD[k]+knowledge_Id])
 	 							/* console.log("缓存单选的选项值："+locationcolor["que"+i+"1"+ABCD[k]+knowledge_Id]); */
 	 						}
 							if(locationcolor["timushocang"+"que"+i+"2"+ABCD[k]+knowledge_Id] != null){//添加缓存颜色
 	 							$("[id = "+"timushocangque"+i+"2"+ABCD[k]+"]").prop("class",locationcolor["timushocang"+"que"+i+"2"+ABCD[k]+knowledge_Id])
 	 						} 
 							
 						}
 						for(var k = 1;k<5;k++){
 							if(locationcolor["timushocang"+knowledge_Id+"que"+i+k+"6"+ABCD[k]] != null){//添加缓存颜色
 	 							for(var j = 1;j<5;j++){
 	 								$("[id = "+"timushocang"+knowledge_Id+"que"+i+j+ABCD[k]+"]").prop("class",locationcolor["timushocang"+knowledge_Id+"que"+i+k+"6"+ABCD[k]])
 	 							}
 	 						}
 							if(locationcolor["timushocang"+knowledge_Id+"que"+i+k+"7"+ABCD[k]] != null){//添加缓存颜色
 	 							for(var j = 1;j<5;j++){
 	 								$("[id = "+"timushocang"+knowledge_Id+"que"+i+j+ABCD[k]+"]").prop("class",locationcolor["timushocang"+knowledge_Id+"que"+i+k+"7"+ABCD[k]])
 	 							}
 	 						}
 						}
					...
					localStorage.removeItem("timushocang"+"que"+knowledge_Id);//删除H5缓存的键（删除该科目 的做题数量记载）
					localStorage.removeItem("timushocang"+"queanswer"+knowledge_Id);
					localStorage.removeItem("timushocang"+"answers"+knowledge_Id);
					var  ABCD = ["A","B","C","D","E"]
 					for(var i = 1;i<= datalength;i++){
 						localStorage.removeItem("timushocangshuanchu"+i+knowledge_Id);
 						for(k = 0;k<5;k++){
 							if(locationcolor["timushocang"+"que"+i+"1"+ABCD[k]+knowledge_Id] != null){//清除选项颜色缓存
 								locationcolor.removeItem("timushocang"+"que"+i+"1"+ABCD[k]+knowledge_Id)
 	 						}
 							if(locationcolor["timushocang"+"que"+i+"2"+ABCD[k]+knowledge_Id] != null){//清除选项颜色缓存
 	 							locationcolor.removeItem("timushocang"+"que"+i+"2"+ABCD[k]+knowledge_Id)
 	 						} 
 							if(locationcolor["timushocang"+knowledge_Id+"que"+i+k+"6"+ABCD[k]] != null){//清除选项颜色缓存
 	 							locationcolor.removeItem("timushocang"+knowledge_Id+"que"+i+k+"6"+ABCD[k])
 	 						}
 							if(locationcolor["timushocang"+knowledge_Id+"que"+i+k+"7"+ABCD[k]] != null){//清除选项颜色缓存
 	 							locationcolor.removeItem("timushocang"+knowledge_Id+"que"+i+k+"7"+ABCD[k])
 	 						}
					 ...
					 for(var i = 1;i<= datalength;i++){
						/* console.log("删除缓存值："+i); */
						locationABCDE.removeItem("timushocang"+"answers"+i+knowledge_Id);
						locationABCDE.removeItem("timushocang"+"que"+i+knowledge_Id);
						locationABCDE.removeItem("timushocang"+"que"+i+"answer"+knowledge_Id);
						locationABCDE.removeItem("timushocang"+"answer"+i+"s"+knowledge_Id); 
						/* console.log("删除后的值："+locationABCDE["que"+i+"answer"+knowledge_Id]（这种方法是定义一个H5缓存 的键）); */
						for(var DB = 1;DB<5;DB++){
							 localStorage.removeItem("timushocang"+"que"+i+"answer"+DB+knowledge_Id);
						}
					 } 
            ...
            $.each(datas, function (i, item) {//each是jquery的遍历方法。data就是传入的用来循环的题目数据与c:forEach中从后台传入的集合变量类似。item与c:forEach中var的用来循环集合值的变量
            	 ...
            	 if(item.questions_Type == 1 || item.questions_Type == 2){
                   ...
                   $.each(questions_json.options,function (x, option) {																																																																				 							  
                     ...
                     for(var l = 0;l<abcdsz.length;l++){//题干
                    	/*  alert("循环选项时数组abcdsz的长度："+abcdsz.length); */
                     ...
                     for(var j = 0 ;j<answerAnalysisbx.length;j++){//循环出下面的小题
                    	 ...
                    	 /* console.log("字符串："+answerAnalysisbxt);  */
                         ...
                         for(var e = 0;e<5;e++){//ABCDE选项																			//knowledge_Id+que11A
                        	 questrhtml += "<div class=\"queoption\"  tag=\"" + optiondata[e]+ "\"><span class=\"optionspan\"  id=\""+knowledge_Id+dataid[i]+DiBiao+optiondata[e]+"\" ><div  class=\"topic\"  id=\"" + dataid[i] + "\" ><div class=\"DiBiao\"  id=\""+DiBiao+"\"><div class=\"length\" id=\""+answerAnalysisbx.length+"\"></div></div><div  id=\"answer\"  class=\""+questions_Json.answer+"\"  tag=\"" + optiondata[e]  + "\"></div><div  class=\"" + questions_Jsonjiexi.answerAnalysis + "\"   id=\"answerAnalysis\"></div><div  class=\"jiexigeshu\"  id=\""+jiexilength+"\"></div>"+item.questions_Type+"</div>" + optiondata[e] + "</span></div>";//optiondata是设置选项数组，用角标循环出来。option是data里定义的题目内容 */
                            }
                     }
                     questrhtml += "</div></div>";
				...
				/* 	} */
                    ...
                    /*    if(questionsJson.title.search("http") != -1 || questionsJson.title.search("https") != -1){ */
						 ...
						 /* }    */
                         for(var e = 0;e<5;e++){
                        		questrhtml += "<div class=\"queoption\"  tag=\"" + optiondata[e]+ "\"><span class=\"optionspan\"  id=\""+knowledge_Id+dataid[i]+DiBiao+optiondata[e]+"\" ><div  class=\"topic\"  id=\"" + dataid[i] + "\" ><div class=\"DiBiao\"  id=\""+DiBiao+"\"><div class=\"length\" id=\""+answerAnalysisbx.length+"\"></div></div><div  id=\"answer\"  class=\""+questionsJson.answer+"\"  tag=\"" + optiondata[e]  + "\"></div><div  class=\"" + questions_Jsonjiexi.answerAnalysis + "\"   id=\"answerAnalysis\"></div><div  class=\"jiexigeshu\"  id=\""+jiexilength+"\"></div>"+item.questions_Type+"</div>" + optiondata[e] + "</span>"+questionsJson.options[e]+"</div>";//optiondata是设置选项数组，用角标循环出来。option是data里定义的题目内容 */
                           }
                      }
                     questrhtml += "</div></div>";
				  }
    	 ...
    	 /*  alert("进入方法"); */
    	  ...
    	  var  kemuZYHXY = $(".kemuZYHXY").text();
        	...
        	for(var z = 1;z<= datalength ;z++){
        		que[z - 1] = locationABCDE["timushocang"+"que"+z+knowledge_Id];
        	}
        	
        	for(var i = 1 ;i<= datalength ;i++){//判断题目正确率
		            if(locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] != null && locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] != ""){
		        	   if(locationABCDE["timushocang"+"answers"+i+knowledge_Id] == Array.from(locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id]).sort().join("")){//进行比较正确的为0，不正确的为1
		        		    locationABCDE["timushocang"+"answer"+i+"s"+knowledge_Id]  =  0;
		               		  successrate[i - 1] = locationABCDE["timushocang"+"answer"+i+"s"+knowledge_Id];
		               	}else if(locationABCDE["timushocang"+"answers"+i+knowledge_Id] != Array.from(locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id]).sort().join("")){
		               		locationABCDE["timushocang"+"answer"+i+"s"+knowledge_Id]  =  1;
		               		  successrate[i - 1] = locationABCDE["timushocang"+"answer"+i+"s"+knowledge_Id];
		       			}
       				 }else{
        	...
        	//对用户选择的选项进行排序
        	 ...
        	 console.log("长度datalength的值："+datalength);
        	for(var j = 1;j<= datalength ; j++){
        		locationABCDE["timushocang"+"que"+j+"answer"+knowledge_Id] = Array.from(locationABCDE["timushocang"+"que"+j+"answer"+knowledge_Id]).sort().join("");
        		queanswer[j] = locationABCDE["timushocang"+"que"+j+"answer"+knowledge_Id];
        		console.log("赋值完成");
        					  /*  locationABCDE["lishi"+"que"+i+"answer"+knowledge_Id] */
        	} 
        	console.log("比较完成");
        	 //该题正确的答案//
        	 var   answers = [];
        	for(var k = 1;k<= datalength;k++){
        		answers[k] = locationABCDE["timushocang"+"answers"+k+knowledge_Id];
        	} 
          ...
          /*   return; */
        	 for(var t = 1;t <= datalength;t++){
        		if(queanswer[t] == null || queanswer[t] == ""){
		        	...
		        	storagexuanxian["timushocang"+"queanswer"+knowledge_Id] = queanswer; //用户选择的答案
		        	storagexuanxian["timushocang"+"answers"+knowledge_Id] = answers; //正确的答案
		        	var  kemuZYHXY  = $(".kemuZYHXY").text();
		        	console.log("正确答案："+storagexuanxian["answers"]);
		        	/* alert("章节id："+knowledge_Id); */
          ...
          function  jiexizhu(correctnumber,percentage,time_s,subListname,nickname,openid,headimgurl,tixing,jiexihuidiao,days,dateTime,integral,lv1state,lv2state,lv3state,datalength,knowledge_Id,kemuZYHXY){
        	  window.location.href="../timushocangjiaojuan/detailspage?correctnumber="+correctnumber+"&percentage="+percentage+"&int_second="+time_s+"&knowledge_Name="+subListname+"&nickname="+nickname+"&openid="+openid+"&headimgurl="+headimgurl+"&tixing="+tixing+"&jiexihuidiao="+jiexihuidiao+"&days="+days+"&dateTime="+dateTime+"&integral="+integral+"&lv1state="+lv1state+"&lv2state="+lv2state+"&lv3state="+lv3state+"&datalength="+datalength+"&knowledge_Id="+knowledge_Id+"&kemuZYHXY="+kemuZYHXY+"";  
            }   
                   ...
                   if(types == 2){//如果type为2
                	   ...
                	   if(options == "2A"){
                		   if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
                			   ...
                			   locationcolor.removeItem("timushocang"+que+"2A"+knowledge_Id);
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   locationcolor["timushocang"+que+"2A"+knowledge_Id] =  $("[id = "+que+"2A]").prop("class"); 
							}
						}
						if(options == "2B"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+que+"2B]").removeClass("rightOK");
								locationcolor.removeItem("timushocang"+que+"2B"+knowledge_Id);
							}else{
								$(this).find("#"+dataid).addClass("rightOK");
								locationcolor["timushocang"+que+"2B"+knowledge_Id] =  $("[id = "+que+"2B]").prop("class");
							}
						}
						if(options == "2C"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+que+"2C]").removeClass("rightOK");
								locationcolor.removeItem("timushocang"+que+"2C"+knowledge_Id);
							}else{
								$(this).find("#"+dataid).addClass("rightOK");
								locationcolor["timushocang"+que+"2C"+knowledge_Id] =  $("[id = "+que+"2C]").prop("class");
							}
						}
						if(options == "2D"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+que+"2D]").removeClass("rightOK");
								locationcolor.removeItem("timushocang"+que+"2D"+knowledge_Id);
							}else{
								$(this).find("#"+dataid).addClass("rightOK");
								locationcolor["timushocang"+que+"2D"+knowledge_Id] =  $("[id = "+que+"2D]").prop("class");
							}
						}
						if(options == "2E"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+que+"2E]").removeClass("rightOK");//移除e选项 
								locationcolor.removeItem("timushocang"+que+"2E"+knowledge_Id);
							}else{
								$(this).find("#"+dataid).addClass("rightOK");
								locationcolor["timushocang"+que+"2E"+knowledge_Id] =  $("[id = "+que+"2E]").prop("class");
							}
						}
                     }
					 if(types == 1){//如果type为1
						    if(options == "1A"){
								if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
									...
									locationcolor.removeItem("timushocang"+que+"1A"+knowledge_Id);
								}else{
								   $(this).find("#"+dataid).addClass("rightOK");
								   ...
								   locationcolor["timushocang"+que+"1A"+knowledge_Id] =  $("[id = "+que+"1A]").prop("class");
								   locationcolor.removeItem("timushocang"+que+"1B"+knowledge_Id);
								   locationcolor.removeItem("timushocang"+que+"1C"+knowledge_Id);
								   locationcolor.removeItem("timushocang"+que+"1D"+knowledge_Id);
								   locationcolor.removeItem("timushocang"+que+"1E"+knowledge_Id);
							...
							if(options == "1B"){
								if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
									...
									$("[id = "+que+"1D]").removeClass("rightOK");
									$("[id = "+que+"1E]").removeClass("rightOK");
									locationcolor["timushocang"+que+"1B"+knowledge_Id] =  $("[id = "+que+"1B]").prop("class");
									locationcolor.removeItem("timushocang"+que+"1A"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1C"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1D"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1E"+knowledge_Id);
							...
							if(options == "1C"){
								if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
									...
									$("[id = "+que+"1D]").removeClass("rightOK");
									$("[id = "+que+"1E]").removeClass("rightOK");
									locationcolor["timushocang"+que+"1C"+knowledge_Id] =  $("[id = "+que+"1C]").prop("class");
									locationcolor.removeItem("timushocang"+que+"1A"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1B"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1D"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1E"+knowledge_Id);
							...
							if(options == "1D"){
								if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
									...
									$("[id = "+que+"1C]").removeClass("rightOK"); 
									$("[id = "+que+"1E]").removeClass("rightOK"); 
									locationcolor["timushocang"+que+"1D"+knowledge_Id] =  $("[id = "+que+"1D]").prop("class");
									locationcolor.removeItem("timushocang"+que+"1A"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1B"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1C"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1E"+knowledge_Id);
							...
							if(options == "1E"){
								if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
									...
									$("[id = "+que+"1C]").removeClass("rightOK");
									$("[id = "+que+"1D]").removeClass("rightOK");
									locationcolor["timushocang"+que+"1E"+knowledge_Id] =  $("[id = "+que+"1E]").prop("class");
									locationcolor.removeItem("timushocang"+que+"1A"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1B"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1C"+knowledge_Id);
									locationcolor.removeItem("timushocang"+que+"1D"+knowledge_Id);
					...
					if(types == 6){//如果types等于6那么是b型题
					   ...
					   if(options == "6A"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								...
								locationcolor.removeItem("timushocang"+dataidA+"6A");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   ...
							   locationcolor["timushocang"+dataidA+"6A"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidA+"6B");
							   locationcolor.removeItem("timushocang"+dataidA+"6C");
							   locationcolor.removeItem("timushocang"+dataidA+"6D");
							   locationcolor.removeItem("timushocang"+dataidA+"6E");
							}
						}
					   if(options == "6B"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项 
								locationcolor.removeItem("timushocang"+dataidB+"6B");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   ...
							   locationcolor["timushocang"+dataidB+"6B"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidB+"6A");
							   locationcolor.removeItem("timushocang"+dataidB+"6C");
							   locationcolor.removeItem("timushocang"+dataidB+"6D");
							   locationcolor.removeItem("timushocang"+dataidB+"6E");
							}
						}
					   if(options == "6C"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项 
								locationcolor.removeItem("timushocang"+dataidC+"6C");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   ...
							   locationcolor["timushocang"+dataidC+"6C"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidC+"6A");
							   locationcolor.removeItem("timushocang"+dataidC+"6B");
							   locationcolor.removeItem("timushocang"+dataidC+"6D");
							   locationcolor.removeItem("timushocang"+dataidC+"6E");
							}
						}
					   if(options == "6D"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项 
								locationcolor.removeItem("timushocang"+dataidD+"6D");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   ...
							   locationcolor["timushocang"+dataidD+"6D"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidD+"6A");
							   locationcolor.removeItem("timushocang"+dataidD+"6B");
							   locationcolor.removeItem("timushocang"+dataidD+"6C");
							   locationcolor.removeItem("timushocang"+dataidD+"6E");
							}
						}
					   if(options == "6E"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项 
								locationcolor.removeItem("timushocang"+dataidE+"6D");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   ...
							   locationcolor["timushocang"+dataidE+"6E"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidE+"6A");
							   locationcolor.removeItem("timushocang"+dataidE+"6B");
							   locationcolor.removeItem("timushocang"+dataidE+"6C");
							   locationcolor.removeItem("timushocang"+dataidE+"6D");
							}
						}
					...
					if(types == 7){
						...
						if(options == "7A"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项 
								locationcolor.removeItem("timushocang"+dataidA+"7A");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   $("[id = "+dataidA+"B]").removeClass("rightOK"); //$("[id!=1A]"),jquery中根据属性值比较进行元素的获取
							   $("[id = "+dataidA+"C]").removeClass("rightOK"); 
							   $("[id = "+dataidA+"D]").removeClass("rightOK"); 
							   $("[id = "+dataidA+"E]").removeClass("rightOK"); 
							   locationcolor["timushocang"+dataidA+"7A"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidA+"7B");
							   locationcolor.removeItem("timushocang"+dataidA+"7C");
							   locationcolor.removeItem("timushocang"+dataidA+"7D");
							   locationcolor.removeItem("timushocang"+dataidA+"7E");
							}
						}
					   if(options == "7B"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项 
								locationcolor.removeItem("timushocang"+dataidB+"7B");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   $("[id = "+dataidB+"A]").removeClass("rightOK"); //$("[id!=1A]"),jquery中根据属性值比较进行元素的获取
							   $("[id = "+dataidB+"C]").removeClass("rightOK"); 
							   $("[id = "+dataidB+"D]").removeClass("rightOK"); 
							   $("[id = "+dataidB+"E]").removeClass("rightOK");
							   locationcolor["timushocang"+dataidB+"7B"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidB+"7A");
							   locationcolor.removeItem("timushocang"+dataidB+"7C");
							   locationcolor.removeItem("timushocang"+dataidB+"7D");
							   locationcolor.removeItem("timushocang"+dataidB+"7E");
							}
						}
					   if(options == "7C"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项
								locationcolor.removeItem("timushocang"+dataidC+"7C");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   $("[id = "+dataidC+"A]").removeClass("rightOK"); //$("[id!=1A]"),jquery中根据属性值比较进行元素的获取
							   $("[id = "+dataidC+"B]").removeClass("rightOK");
							   $("[id = "+dataidC+"D]").removeClass("rightOK");
							   $("[id = "+dataidC+"E]").removeClass("rightOK"); 
							   locationcolor["timushocang"+dataidC+"7C"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidC+"7A");
							   locationcolor.removeItem("timushocang"+dataidC+"7B");
							   locationcolor.removeItem("timushocang"+dataidC+"7D");
							   locationcolor.removeItem("timushocang"+dataidC+"7E");
							}
						}
					   if(options == "7D"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项 
								locationcolor.removeItem("timushocang"+dataidD+"7D");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   $("[id = "+dataidD+"A]").removeClass("rightOK"); //$("[id!=1A]"),jquery中根据属性值比较进行元素的获取
							   $("[id = "+dataidD+"B]").removeClass("rightOK"); 
							   $("[id = "+dataidD+"C]").removeClass("rightOK"); 
							   $("[id = "+dataidD+"E]").removeClass("rightOK"); 
							   locationcolor["timushocang"+dataidD+"7D"] =  $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidD+"7A");
							   locationcolor.removeItem("timushocang"+dataidD+"7B");
							   locationcolor.removeItem("timushocang"+dataidD+"7C");
							   locationcolor.removeItem("timushocang"+dataidD+"7D");
							}
						}
					   if(options == "7E"){
							if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
								$("[id = "+dataid+"]").removeClass("rightOK");//移除a选项 
								locationcolor.removeItem("timushocang"+dataidE+"7E");
							}else{
							   $(this).find("#"+dataid).addClass("rightOK");
							   $("[id = "+dataidE+"A]").removeClass("rightOK"); //$("[id!=1A]"),jquery中根据属性值比较进行元素的获取
							   $("[id = "+dataidE+"B]").removeClass("rightOK"); 
							   $("[id = "+dataidE+"C]").removeClass("rightOK"); 
							   $("[id = "+dataidE+"D]").removeClass("rightOK"); 
							   locationcolor["timushocang"+dataidE+"7E"] = $("[id = "+dataid+"]").prop("class");
							   locationcolor.removeItem("timushocang"+dataidE+"7A");
							   locationcolor.removeItem("timushocang"+dataidE+"7B");
							   locationcolor.removeItem("timushocang"+dataidE+"7C");
							   locationcolor.removeItem("timushocang"+dataidE+"7D");
							}
						}
					}
	           			...
	           			var	option = options.replace("1",""); //因为获取的选项值中带有type，因此去掉type值
	           			 for (var i = 1; i <= datalength; i++) {
	           				if("que"+i == $(this).find(".topic").prop("id")){//判断当前页面是哪个页面（哪道题）
	           					locationABCDE["timushocang"+"answers"+i+knowledge_Id]  = $(this).find("#answer").prop("class");//本题正确答案
	           					locationABCDE["timushocang"+"answerAnalysis"+i+knowledge_Id] = $(this).find("#answerAnalysis").prop("class");//本题解析
	           					locationABCDE["timushocang"+"que"+i+knowledge_Id] = $(this).find(".topic").prop("id");//给当前页面的id赋值，用于判断选择是否正确
	           					...
	           					onanswer = "a";
	           					if(option == locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id]){//如果在次点击同一个选项，就移除que1answer和doubleanswer的值。因此他为空了是相等的，所以不会在重复添加
	               					locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id].replace(option,"");//replace移除字符串中指定的字符。可以传入要移除字符的参数
	               				...
	               				if(doubleanswer1 != locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id]){//因为doubleanswer会先赋值因此正常情况下会比que1answer多一个值，只有在上面清除了值的情况下才会相等
	               					locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] = option;//将获取到的点击的选项的值(ABCD)，给与变量，用于答案比较
	               					console.log("赋值后的值："+locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id]);
	               				}
	           				}
	           				  locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] =  locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id];
	           				  onanswer  = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id];
	           			 }
	           		   }
	           		if(types == 2){//如果type为二，就是多选
	           			...
	           			for (var i = 1; i <= datalength; i++) {
	           			  if ("que"+i == $(this).find(".topic").prop("id")) {
	           			  	locationABCDE["timushocang"+"answers"+i+knowledge_Id] = $(this).find("#answer").prop("class");//获取当前页面题目的真确答案
	           				locationABCDE["timushocang"+"answerAnalysis"+i+knowledge_Id] = $(this).find("#answerAnalysis").prop("class");//本题解析
	           			    locationABCDE["timushocang"+"que"+i+knowledge_Id] = $(this).find(".topic").prop("id");//获取当前页面id值，给与变量用于题目答案判断
	           			    if(ques != "que"+i){
	        					ques = "que"+i;
	        					doubleanswer2 = "";
	        					queanswer  = "";
	           			    ...
	           			    if(queanswer.search(option) != -1){//search方法用来判断字符串中某个字符是否存在，存在就不等于-1(这个时候变量里就有了传进去的数据，所以就)
	           			    	/* alert("清除"); */
	           			    	locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id].replace(option,"");
	           					doubleanswer2 = doubleanswer2.replace(new RegExp(option,"gm"),"");
	           					queanswer = queanswer.replace(new RegExp(option,"gm"),"");
	           					if(locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] == null){
	           					    console.log("取多选的选项");
	           					}
	           				}
	           				  /*  alert("i的值："+i);  */ 
	           				/* alert("下一个多选的值："+locationABCDE["que"+i+"answer"+knowledge_Id]); */
	           			    if(doubleanswer2 != locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] && doubleanswer2 != null && doubleanswer2 != ""){//与上面类似
	           			    	/*  alert("进入添加值后初始值："+locationABCDE["que"+i+"answer"+knowledge_Id]);  */ 
	           			    	if(locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] == null){//window["que"+i+"answer"]没付值时是null
	           						locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] = option;
	           					}else {
	           						locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id]+option;
	           						locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id].replace("undefined","");
								}
	           					    queanswer = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id];
	           					}
	           			 		    locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] =  locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id];
	           			 		    onanswer = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id];
	           			  	 }
	            	 ...
	            	 if(types == 6){//等于6那么就是B型题//B型题如果没有把所有小题做完就默认是没做
	            		...
	            		for (var i = 1; i <= datalength; i++) {
	          			 if ("que"+i == $(this).find(".topic").prop("id")){//如果点击的当前的div是
	          			   ...
	          			   locationABCDE["timushocang"+"answerAnalysis"+i+knowledge_Id] = answerAnalysis6 //本题解析
	          			   locationABCDE["timushocang"+"que"+i+knowledge_Id] = $(this).find(".topic").prop("id");//获取当前页面id值，给与变量用于题目答案判断
	          			   doubleanswer6  =   option;  //会先给doubleanswer赋予选择的选项值
	          			   if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
		          				 if(locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] == null){
		          					locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] = option;
		       						DiBiaos = DiBiao;
		       					    dataids = dataid;//dataid是当前点击的选项的id*/
		       					 	ques =  $(this).find(".topic").prop("id"); 
		       						  ...
		       						  if(DiBiaos != DiBiao && dataids != dataid  && locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] != null){
		       							locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] =  option;
		       							 ques =  $(this).find(".topic").prop("id");  
		          				 ...
		          				 locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] = locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id].replace(option,"");
		          				 /* alert("删除掉一个选项："+ window["que"+i+"answer"+DiBiao]) */
		          					doubleanswer6 = "";
		          					dataids = "";
		          					DiBiaos = "";
		          					queianswerf  =  [];
		          					locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] = "";
							}
	          			 	queianswerf[DiBiao-1]  =  locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id]
	          			    /*  alert("queianswerf值"+queianswerf[DiBiao-1]);  */
	          			     if(queianswerf.length == blength){//如果数组的长度与此b型题长度相同说明说以题都选了
	          			    	 /* alert("数组长度等于小题长度"); */
	          			    	 locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] = "";//定义一个数据变量
	          			    	...
	          			    	for(var j = 0;j< blength;j++){//如果数组的值中没有空值
	          			    		locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] += queianswerf[j];//循环将数组给与变量
	          			    		onanswer = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id];
		          			     }
	          			     }
	          			   /*    alert("window值"+window["que"+i+"answer"]);   */   
	          			  locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] =  locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id];
	          			  locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] = "";
	          			  }
	            		}	
	            	} 
	            	 ...
	            	 if(types == 7){//等于7那么就是综合题//综合型题如果没有把所有小题做完就默认是没做
		            		...
		            		for (var i = 1; i <= datalength; i++) {
		          			 if ("que"+i == $(this).find(".topic").prop("id")){//如果点击的当前的div是
			          			...
			          			locationABCDE["timushocang"+"answerAnalysis"+i+knowledge_Id] = answerAnalysis7 //本题解析
		          				locationABCDE["timushocang"+"que"+i+knowledge_Id] = $(this).find(".topic").prop("id");//获取当前页面id值，给与变量用于题目答案判断
		          			   doubleanswer6  =   option;  //会先给doubleanswer赋予选择的选项值
		          			   if($(this).find("#"+dataid).prop("class") == "optionspan rightOK"){
			          				 if(locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] == null){
			          					locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] = option;
			       						...
			       						/* alert("添加后的queanswerDiBiaozonhes值："+window["que"+i+"answer"+DiBiao]); */
			       						  ...
			       						  if(DiBiaozonhes == DiBiao  && dataidzonhes != dataid){
			       							locationABCDE["timushocang"+"que"+i+"answer"+DiBiaozonhes+knowledge_Id] =  option;
			       							ques =  $(this).find(".topic").prop("id");
			       							/* alert("同一道题的选项变化："+window["que"+i+"answer"+DiBiao])  */
			       						   }
			       						  if(DiBiaozonhes != DiBiao && dataidzonhes != dataid  && locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] != null){
			       							locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id] =  option;
			       							ques =  $(this).find(".topic").prop("id");
			       							/* alert("不是同一道题的选项变化："+window["que"+i+"answer"+DiBiao])  */
			       						  }
		          			 ...
		          			 queianswerzh[DiBiao-1] = locationABCDE["timushocang"+"que"+i+"answer"+DiBiao+knowledge_Id]
		          			    /*  alert("queianswerf值"+queianswerf[DiBiao-1]);  */
		          			     if(queianswerzh.length == blength){
		          			    	...
		          			    	for(var j = 0;j< blength;j++){
		          			    		locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] += queianswerzh[j];
		          			    		onanswer = locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id];
			          			     } 
		          			     }  
		          			      /*  alert("window值"+window["que"+i+"answer"]); */
       ...
       $(".btnanniu").click(function () {//用选择器获取到上一题按钮，给与点击事件
        ...
        var  queids = "";
         ...
         function quepageFun(btnpageobj){
          ...
          if(locationABCDE["timushocang"+"que"+onpage+"answer"+knowledge_Id] == null  &&  Dpage == "Dpage"  || locationABCDE["timushocang"+"que"+onpage+"answer"+knowledge_Id] == "undefined" &&   Dpage == "Dpage" || locationABCDE["timushocang"+"que"+onpage+"answer"+knowledge_Id] == "" &&  Dpage == "Dpage" ){
     ...
     function jiexi(){
    	/*  alert("进入解析按钮");
    	 alert("queid的值："+queids); */
     	 ...
     	 /* alert("现在的题目数："+i); */
    	 	//如果这道题没有答，那么给与提示 
    	 if(locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] == null  || locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] == "undefined" || locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id] == "" ){
        	 /*   console.log("进入提示后onpage的值："+onpage); */
 					var layerindex=layer.confirm('您这道题还没有做完!', 
								{
							      title:'提醒',//
    				...
    				return;
      				...
      				/* alert("有答题记录"); */
      				$("#rowjiexi").css("display","block");//有答题记录的情况下改变按钮颜色
      				$("#my").text(locationABCDE["timushocang"+"que"+i+"answer"+knowledge_Id]);
    				$("#correct").text( locationABCDE["timushocang"+"answers"+i+knowledge_Id]);
      			    $("#cankaojiexi").text(locationABCDE["timushocang"+"answerAnalysis"+i+knowledge_Id]);
				}
     		}
    </script>
