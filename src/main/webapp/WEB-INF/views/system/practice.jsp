	 ...
	 //页面加载时就会加载这个函数，然后会调用下面的方法
			...
			$.ajax({
					...
					success:function(data){
						if(data.type == 'success'){
							...
							/* var i = 0; 
							/* var  questions_json  = JSON.parse(datas[3].questions_json.answers);   */
							/* QueInIt(datas,optiondata,dataid); */
							/* console.log("题型名："+datas[0].knowledge_Name);
							  console.log("传过来的json:"+datajson); */
						    /* console.log("subList值："+subList); */
						     /* console.log("第三个选择题的正确选项："+questions_json  = JSON.parse(datas[3].questions_json.answers));  */
        ...
        function QueInIt(optiondata,datas,dataid,chooses,knowledge_Name){
            ...
            $.each(datas, function (i, item) {//each是jquery的遍历方法。data就是传入的用来循环的题目数据与c:forEach中从后台传入的集合变量类似。item与c:forEach中var的用来循环集合值的变量
            	 ...
            	 /* if(titles.search("http") != -1){//判断题目提标title中是否有htttp这个字符串，如果有那么这个题目的提标就是图片
                ...
                if(option.search("http") != -1){
                		//图片
                	var http = https[x].split(">")[1];//1后面的，0前面的
        ...
        function  Jexi(knowledge_Name,datas){
        	...
        	console.log("排序后的值："+Array.from(que1answer).sort().join(""));
         	console.log("answers1的值："+answers1);
        	
        	
        	if(answers1 == Array.from(que1answer).sort().join("")){
        		var  answer1s =  0;
        	}else if(answers1 != Array.from(que1answer).sort().join("")){
        		var  answer1s =  1;
			}
        	
        	if(answers2 == Array.from(que2answer).sort().join("")){
        		var  answer2s =  0;
        	}else if(answers2 != Array.from(que2answer).sort().join("")){
        		var  answer2s =  1;
			}
        	if(answers3 == Array.from(que3answer).sort().join("")){
        		var  answer3s =  0;
        	}else if(answers3 != Array.from(que3answer).sort().join("")){
        		var  answer3s =  1;
			}
        	if(answers4 == Array.from(que4answer).sort().join("")){
        		var  answer4s =  0;
        	}else if(answers4 != Array.from(que4answer).sort().join("")){
        		var  answer4s =  1;
			}
        	if(answers5 == Array.from(que5answer).sort().join("")){
        		var  answer5s =  0;
        	}else if(answers5 != Array.from(que5answer).sort().join("")){
        		var  answer5s =  1;
			}
        	var answer1s ;
        	var answer2s ;
        	...
        	for(var i = 0;i<5;i++){
        		console.log("successrate的值："+successrate[i]);
        	}
        	/* alert("successrate的长度："+successrate);
        	alert("successrate的长度："+successrate[0]);
        	alert("successrate的长度："+successrate[1]);
        	alert("successrate的长度："+queanswer); */
            /* for(var i = 0;i<5;i++){ */
           		...
           		if(types == 2){//如果type为二，就是多选
           			...
           			if("que1" == $(this).find(".topic").prop("id")){
           				...
           				if(que1answer.search(option) != -1){//search方法用来判断字符串中某个字符是否存在，存在就不等于-1(这个时候变量里就有了传进去的数据，所以就)
           					que1answer = que1answer.replace(option,"");//单纯的replace方法只去除字符串中的第一个为option的字符串
           					doubleanswer2 = doubleanswer2.replace(new RegExp(option,"gm"),"");//replace(new RegExp(option,"gm"),"")方法会把所有option字符串去除
           				} 
