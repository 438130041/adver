...
<script type="text/javascript">
	...
	/* angular.module('myApp', [])
		.controller('MyController',function($scope){//这里形参必须是$scope（这个是隐式依赖注入声明）
			$scope.empName = "kobe";
		})
		...
		//以上代码的问题:
			...
			//代码压缩的$scope会被abcd代替，angular无法解析
				//解决方案
		angular.module('myApp',[])
				.controller('MyController',['$scope',function($scope){//不管压缩成什么，前面的$scope都会替代后面的$scope，那么后面一直后是$scope不会变为别的。这就是显示声明依赖注入
					$scope.empName = "kobe";
				}])
