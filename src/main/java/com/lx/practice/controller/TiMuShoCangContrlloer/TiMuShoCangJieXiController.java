...
@Controller
@RequestMapping("/timushocangjiexi")
public class TiMuShoCangJieXiController {
		
	
	//错题重做题目解析
			@RequestMapping(value="/timushocangjiexi",method=RequestMethod.GET)
			 ...
			 // 先对用户名进行解码得到%E7%8E%8B%E6%8C%AF%E5%9B%BD 这样的形式
				...
				if (tixing.equals("中药")) {
					...
					if ("中药学专业知识一".equals(Knowledge_Name)) {
						...
						model.setViewName("timushocang/lishicuotichonzuojiexi");
				...
				if (jiexihuidiao.equals("jiexihuidiao")) {
				    ...
				    nickname = URLDecoder.decode(nickname, "UTF-8");
						...
						if(tixing.equals("中药")){
							...
							model.setViewName("timushocang/shocangjingxuandetailspage");
						}
						if(tixing.equals("西药")){
							System.out.println("西药");
							model.setViewName("timushocang/shocangjingxuandetailspageWest");
						}
				  }else{
